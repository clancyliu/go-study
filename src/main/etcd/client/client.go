...
func grpcCall() {
	...
	defer cancel()
	r, err := client.SayHello(ctx, &proto.HelloRequest{Name: "clancy"})
	if err != nil {
...
func invoke() {
	...
	err = conn.Invoke(context.Background(), "/proto.Greeter/SayHello", &proto.HelloRequest{Name: "clancy"}, &response)
	if err != nil {
		log.Fatalln(err)
	}
